...
class CRUDAndValidationTests(APITestCase):
    ...
    def test_FT_01_buat_laporan_dengan_data_lengkap(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, "Harus bisa buat laporan")
        self.assertEqual(Report.objects.count(), 1, "Jumlah laporan harus 1")
    ...
    def test_FT_02_ditolak_jika_judul_kosong(self):
        ...
        response = self.client.post(url, payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST, "Harus ditolak jika title kosong")
        self.assertIn('title', response.data, "Error response harus menyebutkan title")
    ...
    def test_FT_03_ditolak_jika_deskripsi_kosong(self):
        ...
        response = self.client.post(url, payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST, "Harus ditolak jika description kosong")
        self.assertIn('description', response.data, "Error response harus menyebutkan description")
