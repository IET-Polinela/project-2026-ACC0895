...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        ...
        self.assertEqual(
            ...
            "Warga tidak boleh mengedit laporan yang sudah REPORTED (harus 403)"
        )
    ...
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        ...
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Warga tidak boleh mengedit laporan yang sudah RESOLVED (harus 403)"
        )
...
class AdminWorkflowTests(TestCase):
    ...
    def test_WF_03_admin_mengubah_status_reported_ke_verified(self):
        ...
        url = reverse('verify_report', kwargs={'pk': self.laporan_reported.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 302, "Harus redirect setelah mengubah status")
        self.laporan_reported.refresh_from_db()
        self.assertEqual(self.laporan_reported.status, 'VERIFIED', "Status harus berubah menjadi VERIFIED")
    ...
    def test_WF_04_tidak_ada_transisi_langsung_ke_resolved_dari_reported(self):
        ...
        self.assertNotIn(reverse('resolve_report', kwargs={'pk': self.laporan_reported.pk}), response.content.decode(), "Tidak boleh ada tombol resolve langsung")
        self.assertIn(reverse('verify_report', kwargs={'pk': self.laporan_reported.pk}), response.content.decode(), "Harus ada tombol verify")